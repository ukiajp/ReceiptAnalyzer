...
### Claude Code のオーケストレーター実行ルール

**Claude Code はオーケストレーターとして、以下を自分で Bash 直接実行する。ユーザーにコマンドを渡して実行させてはならない。**
...
# agy（Gemini）へのレビュー依頼
python C:\Users\ukyas\dev\agy_runner.py "プロンプト"
```

実行順序：
1. 仕様書（doc/）を確認・更新する
2. `codex exec` で実装を生成させる
3. `agy_runner.py` で敵対レビューを行う
4. agyの指摘を確認し、ユーザーにGATEを求める
5. 承認後にコミット・プッシュする
...
4. 入力源はマウント済みフォルダ経由とする（Drive API は使用しない・ADR-012）
